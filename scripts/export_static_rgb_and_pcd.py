...
try:
    ...
    ply = rs.save_to_ply(f"output/{args.filename}.ply")
    ...
    print(f"Saving to {args.filename}.ply...")
    # Apply the processing block to the frameset which contains the depth frame and the texture
    ply.process(frames)
    print("Done")
    
    # read ply file and write to pcd file
    o3d.io.write_point_cloud(f"output/{args.filename}.pcd", 
                             o3d.io.read_point_cloud(f"output/{args.filename}.ply"))
    
    # remove temp.ply file
    # os.remove(f"output/{args.filename}.ply")
